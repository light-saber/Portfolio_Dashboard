...
export default function KPICard({ title, value, sub, delta, deltaLabel, tooltip }) {
  ...
  return (
    <div className="card flex flex-col gap-2 group relative">
      {tooltip && (
        <div className="absolute top-3 right-3 hidden group-hover:block z-10 w-48 bg-white border border-slate-200 text-slate-600 dark:bg-navy-800 dark:text-slate-300 dark:border-slate-700 text-xs rounded-lg p-2.5 shadow-xl">
          {tooltip}
        </div>
      )}
