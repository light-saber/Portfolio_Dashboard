...
@router.get("/all")
async def all_benchmarks(period: str = Query("1Y", pattern="^(1M|3M|6M|1Y|3Y|Max)$")):
    return await fetch_all_benchmarks(period)


@router.get("/{symbol}")
async def single_benchmark(symbol: str, period: str = Query("1Y", pattern="^(1M|3M|6M|1Y|3Y|Max)$")):
